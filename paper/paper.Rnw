...
\mt{\begin{itemize}
\item Goal: Scheme that is broad enough to classify most shoes and specific enough to identify meaningful similarity between visually similar shoes
\item Papers (Gross et all, 2013) suggested geometric features have discriminatory power
\item Multi-label: label all shapes in a region
\item Modified: e.g., polygon out of pentagon and hexagon
\item Refer to table
\end{itemize}}
...
\mt{\begin{itemize}
\item Mention that they're high school and/or undergrad lackeys? (Jk, students)
\item Number of shoe images, number of labels
\item Bar chart?
\end{itemize}}
