...
archetype <- list.files(file.path(wd, "images", "consistency", "single-exemplar"), full.names = T)
pred_prob_plot(archetype, loaded_model, sort = F)
@
...
exemplar <- list.files(file.path(wd, "images", "consistency", "multi-exemplar"), full.names = T)
...
pred_prob_plot(rev(chevron), loaded_model, sort = F)
@
...
pred_prob_plot(rev(circle_text), loaded_model, sort = F)
@
...
\svp{We may want to explore using the border of the square to show whether the model actually assigns that label - like the bullet plots}
