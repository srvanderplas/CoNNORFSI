...
\item Confusion Matrix
  ...
  \item Conf mat doesn't tell us why certain predictions are wrong
  \end{itemize}
\item In this paper, we identify two main reasons for error: some classes are more variable, and some classes share physical features (I.e., the model has to learn a lot of features to encapsulate a class with lots of variety, but then other classes share those features)
\item Brands
  \begin{itemize}
  \item Bowties/polygons/stars are mostly a few brands, but the top quad/line brands still only take up a small proportion of all quads (.04 or less)
  \end{itemize}
\item Heatmaps
  \begin{itemize}
  \item Verify that the model is using appropriate features when predictions are correct
  \item Show examples for strong misclassification, but make the case that the mistakes are understandable
  \end{itemize}
\end{itemize}
}
...
\mt{These figures provide representative examples of classifications across multiple categories; it is also useful to examine many cases within a given shape. \autoref{fig:chevrons-consistency}}
...
\fix{Now that we've seen specific examples, it is useful to summarize the systematic errors by class.} A confusion matrix is a helpful diagnostic tool to understand which classes are commonly confused for other classes.} A confusion matrix is a \mt{summary of the relationships between} the labels assigned to an image and the labels predicted by the model.
