...
%\mt{Training a CNN from scratch is hard. There are CNNs trained on complex images, and transfer learning lets us use those weights. In particular, we're using VGG16. We can look at the filters of VGG16 to support our claim that it will be good for the task.}
...
Many convolutional neural networks have been trained on a subset of images \svp{from ImageNet}; these images are hierarchically labeled, so the same image of a dog may have labels of sky, grass, and trees, and the dog may be labeled as ``mammal'', ``dog'', ``retriever'', and ``golden retriever'' at increasing levels of label complexity \citep{dengImageNetLargeScaleHierarchical}. CNNs trained on ImageNet are optimized for general human-like vision, that is, the ability to recognize a large set of different features simultaneously. As a result, the base of networks trained on these sets are often used for transfer learning, because the initial layers are broadly generalizable to a wide variety of more specific image labeling tasks. \fix{More to be said later.}
...
Thousands of outsole images were scraped from online shoe retail sites and annotated using LabelMe, a tool for image annotation in computer vision problems \citep{labelme}. After annotation, the minimum bounding rectangle of the region \svp{is identified and the image is cropped to that area; subsequently, the cropped image is scaled to 256 x 256 pixels.} During this process, aspect ratio is not preserved, though efforts are made to label regions which are relatively square to minimize the effect of this distortion. To date, \Sexpr{unique(ann_df$base_image) %>% length()} shoes have been labeled, yielding \Sexpr{nrow(dfunion)} multi-label images.
...
\item ROC
  \begin{itemize}
  \item Overall: model performance is better than chance
  \item Classes: broad comparison of class performance
  \end{itemize}
\item Examples
  \begin{itemize}
  \item Show what predictions look like
  \item Introduce the idea that some classifications aren't perfect
  \item Motivate systematic class-by-class confusion
  \end{itemize}
\item Confusion Matrix
  \begin{itemize}
  \item Show patterns in misclassification
  \item Motivates further understanding of common mistakes
  \end{itemize}
\item Three main reasons for error: image contrast, some classes are more variable, and some classes share physical features (I.e., the model has to learn a lot of features to encapsulate a class with lots of variety, but then other classes share those features)
\item Contrast
\item Brands
  \begin{itemize}
  \item Bowties/polygons/stars are mostly found in only a few brands, but the top N quad/line brands only take up a small proportion of all quads/lines (.04 or less)
  \item In general, the most difficult classes to classify are those that are highly variable
  \end{itemize}
\item Heatmaps
  \begin{itemize}
  \item Verify that the model is using appropriate features when predictions are correct
  \item Show examples for confident misclassifications (like Seychelles), but make the case that the mistakes are understandable
  \end{itemize}
  \item (IDK where to put this, but I think it's a really important point that consistency matters more than accuracy. As long as a shape is predicted consistently, that provides information about outsole similarity, even if it's not 100\% correct. But, if phrased wrong, this undermines the claim that our model can replicate the feature detection examiners are doing...)
\end{itemize}
}

\paragraph{Overall Accuracy}

For each image in the test set, the ``true" labels are nine human-assigned labels of 0 or 1 corresponding to the presence or absence of each of the nine shape categories in the image. When the CNN predicts which shapes are in the image, however, it assigns a probability between 0 and 1 for each shape category, with a total of nine probabilities per test image. Thus, to determine the accuracy of the model predictions, we must select a threshold for each label to discretize the model predictions. If the threshold is too low, moderate probabilites will \mt{always meet the threshold}, which increases the true positive and false positive predictions in the test data. Conversely, if the threshold is too high, moderate probabilities will \mt{fail to meet the threshold,} which increases the number of true and false negative predictions. An appropriate threshold must be chosen to produce a high true positive rate while also ideally keeping the false positive rate low. \mt{Is it worth introducing the acronyms TPR and FPR?}

Receiver Operating Characteristic (ROC) curves are a type diagnostic plot which compare the false positive rate to the true positive rate for a classification method, and the Area Under the Curve (AUC) \svp{is a summary measure which} quantifies the \svp{overall accuracy of the model}. %It doesn't really summarize the shape - you can have very different shapes of curves with the same AUC value.
Perfect prediction would be indicated by a right angle along the upper left corner of the plot, with a corresponding AUC of 1, and a diagonal line with an AUC of 0.5 would indicate that the classification method performs no better than random chance. \autoref{fig:overall-roc} shows the ROC curve for \fix{the model} across all classes, and \autoref{fig:class-roc} shows the curve for each class. The full model has an AUC of 0.88, and the AUC for individual classes ranges from 0.81 (for lines) to 0.91 (for bowtie and text). \fix{pull these numbers out of the model using Sexpr} While the class performances do vary slightly, each ROC curve is the same general shape and performs significantly better than random chance. The points in \autoref{fig:class-roc} represent the equal error rate (EER), which is the threshold where there is equal probability of a false positive or false negative error. We will use the class-wise EER to set a threshold corresponding to each class, so that any model prediction with probability above the EER for that shape is considered a positive prediction.
...
The ROC curves provide only a general summary of model accuracy; to learn more about how specific images are classified, we can view images from the test set along with their respective nine model predictions. \autoref{fig:single-exemplars} shows model predictions for images that contain only one type of shape, and \autoref{fig:multi-exemplars} shows predictions for images containing multiple shapes. Most predictions for the single-class images are \fix{promising}; there are some confusions between circle/text and polygon/quad, but all of the true shapes are corectly identified by the model. \fix{The multi-label images also perform quite well; again, true labels are almost all present, with a small number of false positive labels. [Expand on specific cases more, once determining how many examples to keep and how much to highlight errors.] A later section will discuss possible causes for these class confusions.}
...
<<single-exemplars, fig.cap = "Examples containing a single shape", fig.pos = "p!", out.width = "100%", cache = T>>=
single_exemplars <- list.files(file.path(wd, "images", "consistency",
                                         "single-exemplar"), full.names = T)
pred_prob_plot(single_exemplars, loaded_model, sort = F, eer = eers)
@

<<multi-exemplars, fig.cap = "Examples containing multiple shapes", fig.pos = "p!", out.width = "100%", cache = T>>=
multi_exemplars <- list.files(file.path(wd, "images", "consistency",
                                        "multi-exemplar"), full.names = T)
pred_prob_plot(rev(multi_exemplars), loaded_model, sort = F, eer = eers)
@

\fix{Figures \ref{fig:single-exemplars} and \ref{fig:multi-exemplars} provide representative examples of classifications across multiple categories; it is also useful to examine many cases within a given shape.} \autoref{fig:chevrons-consistency} shows a number of images containing chevrons. The top four rows show regularly repeating chevron patterns and seems to indicate that the model optimally identifies chevrons of a certain width; wide chevrons cross into the realm of bowties, whereas narrow chevrons become indistinguishable from lines. The next six rows show individual and closed chevrons. There is a significant amount of confusion between these wider chevrons and quadrilaterals. Finally, the bottom four rows show less-than-standard chevrons. The rounded chevron has a much lower prediction than those with sharper angles, but still exceeds the EER for chevron; the white chevrons are confounded with overlaid circles, which introduces a complexity that the model attributes to text.
...
pred_prob_plot(rev(chevron), loaded_model, sort = F, eer = eers)
@

\fix{Describe \autoref{fig:circle-text-consistency}...... The final two don't contain a true circle. IDK why the TOMS logo totally misses circle.}

\mt{If We end up making the claim that consistency matters more than accuracy, we can use \autoref{fig:ugg-consistency} to show that consistently predicting line isn't strictly right but does establish similarity between the images. If we don't emphasize the consistency point as much, the ugg plot is overkill.}
...
<<ugg-consistency, fig.cap = "Text with circles or circle-like shapes", fig.pos = "p!", out.width = "100%", cache = T>>=
ugg <- list.files(file.path(wd, "images", "consistency", "ugg-star-logo"), full.names = T)
pred_prob_plot(rev(ugg), loaded_model, sort = F, eer = eers)
@
...
\mt{\paragraph{Accuracy by class}}
\fix{Figures \ref{fig:single-exemplars}-\ref{fig:circle-text-consistency} show predictions for a number of individual images. Many of the misclassifications in these examples are not just seen in individual images, but rather exist systematically in a class.} A confusion matrix is a type of visualization that summarizes the relationships between the labels assigned to an image and the labels predicted by the model across all test data, and is a helpful diagnostic tool to understand which classes are systematically misclassified. \mt{The confusion matrix for \fix{our model} is presented in \autoref{fig:confusion_matrix}}. The values along the diagonal represent the proportion of true positives captured within each category. The off-diagonal values represent the proportion of false positives for each shape pairing; however, since a single image may truly contain multiple shapes, the values have been adjusted to remove the effect of any true positives from the calculation of false positive proportions.
%\svp{As a result of this adjustment, the probabilities off-diagonal are the proportion ... need to explain the conditional probability we're showing here}
\fix{As a result of this adjustment, the off-diagonal values are the proportion of images within a true class that are falsely labeled as containing another class that has not been identified in the image. }%\svp{I can't remember exactly how we're doing that, but we may want to look more closely at that - if we set everything to NA instead of 0, the rate calculations would be higher than expected}
For example, to calculate the proportion of images that contain triangles but are being falsely labeled as containing quadrilaterals, any image that truly contains both triangles and quadrilaterals is removed before calculating the proportion of false quadrilateral labels. \mt{Note that this matrix is not symmetric---the proportion of images containing text that are falsely predicted to contain circles is not necessarily equal to the proportion of images containing circles that are falsely predicted to contain text.}
...
The horizontal band in \autoref{fig:confusion_matrix} indicates that quadrilaterals are predicted more often than they should be for every true label. Similarly, polygons, stars, and triangles produce a large number of false positive predictions into other categories, as evidenced by the vertical bands. Another interesting artifact in \autoref{fig:confusion_matrix} is that circles are often misclassified as text, and text is often mislabeled as containing circles.

\fix{(Transition) We have identified three possible causes for systematic misclassification: incorrect labels, low contrast, and variability within classes/overlap between classes.}

%One reason for these misclassifications is that not all of the images used in model training are accurately labeled: in some cases, the labels have not \mt{yet} been updated after labeling guidelines have changed. The confusion between text and circles is a natural consequence of the overlap between geometric shapes and components of written text; it is not indicative of a lack of fit as much as a weakness in the classification system used to define the model.

\paragraph{Contrast}

\fix{Low contrast images are difficult to predict because the features are not prominent. The model has 'learned' that predicting diffuse probabilities across the classes minimizes loss, so it tends to assign diffuse probabilities to low contrast images. Contrast correction seems to improve predictions, but the diffuse probability strategy persists, so we will incorporate contrast correction into model training.}

<<low-contrast-consistency, fig.cap = "Images with low contrast, before and after correction", fig.pos = "p!", out.width = "100%", cache = T>>=
lowcontrast <- list.files(file.path(wd, "images", "consistency", "low-contrast"), full.names = T)
pred_prob_plot(lowcontrast, loaded_model, sort = F, eer = eers)
@

\paragraph{Brand Investigations}

One possible explanation for the poor performance of some categories is the amount of variability within the shapes. \fix{For example, as seen in \autoref{fig:brands-within-shapes}, a large proportion of bowties come from Birks; thus, the model learns bowties from Birks well, which leads to more accurate predictions of new bowtie images that come from Birks. There is no similar dominance in categories like quads. Instead, quads are highly variable  and come from a wide range of brands, so it is harder for the model to learn which set of features designate a quadrilateral. Since a large number of features are then associated with quads, the model gets more easily confused. When it sees some of those same features in other classes, it tries to guess quad. This is further explored in the next section.}

\mt{That was a really quick and dirty version of what I'm trying to say... Am I missing any big points, before I try to expand it and clean up the technical part of the writing?}
...
<<brands-within-shapes, fig.cap = "Caption here">>=
...
Belonging to a category of techniques called class activation map (CAM) visualizations, a heatmap is a visual diagnostic tool that highlights the portions of an image that are most significant to classification for a given class \citep{chollet_allaire_2018}. Using these graphics, we can identify which locations in an image contribute most to the model's predictions. In \fix{the figures in this subsection,} heatmaps are shown for a selection of three classes, which are either contained in the image or have high output probabilities, alongside the original image. \fix{Many of these heatmaps illustrate that multiple classes share certain features, which understandably confuses the model.}

%\fix{I picked the heatmaps I thought illustrated the point best, but didn't modify them heavily yet. How many do we want to include, and do we want to highlight model challenges or just focus on the point that CoNNOR sees what we see?} \svp{I think it's worth highlighting the fact that we'll never get a model that's perfect because that curly s is pretty much a circle, and there will always be weird shapes that partially activate different categories.}

\fix{Susan, please generate these heatmaps and move them to the "static" folder. My computer still has issues with the transparent layer of the heatmap, so it's probably easiest to do what we did for my CC and have you generate the images so they turn out correctly. Once I can see them all, I'll decide which three (or so) heatmaps best illustrate the point that shape categories have similar features that (understandably) confuse the model and I'll write about those specific heatmaps.}
...
\mt{I'll do this once Results is stable, so I can make it match the rest of the story.}
