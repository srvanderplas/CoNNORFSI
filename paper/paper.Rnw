...
Model training is analogous to the human learning process. A child learns to identify dogs by being presented with many labeled examples, where the parent uses the label "dog" for an animal walking by. That child's understanding of dogs is then measured by how many dogs the child is able to correctly identify, and also by the number of cats it mistakenly calls "dog". Similarly, CNN training is a series of stages where, in each, the model learns from a set of labeled training data and that learning is  evaluated through intermediate predictions of validation data.
