...
%\mt{By labeling a set of images within a feature space similar to that used by examiners, then utilizing the generalizability of pre-trained CNN feature detectors to train a model to predict the presence of these features in outsole images, }

\section{Materials and Methods}
\fix{We may want to mention some of the methods in the introduction for continuity sake...}
\mt{I'm not sure if adding a sentence to the above paragraph accomplished this goal. Do you think I need to write a new transition paragraph instead? I tried doing that (see commented out stuff), but wasn't sure how to avoid redundancy with the methods section...}
...
<<current-heatmaps, out.width = ".95\\textwidth", fig.width = 8, fig.height = 6, fig.cap = "Correct or reasonably incorrect heatmaps", echo = F, message = F, warning = F, fig.pos = "!p">>=
...
The goal of this research was to develop a method to automatically identify geometric class characteristics of shoe outsoles. A set of geometric class characteristics was defined, based on feature sets currently used by examiners, to both broadly classify a large variety of shoes and to narrow down similarity into a manageable number and type of features for further use in a shoeprint analysis. Thousands of outsole images were obtained from online shoe retailers and labeled according to their geometric shapes. A model, the \mt{pre-trained} convolutional base of VGG16 and a newly trained classifier/head, was then trained on a portion of these labeled images to output predicted probabilities within the defined set of geometric features. After training, the model performs well on the data provided. In general, the model is able to identify many well-defined geometric shapes in the images.

%The goal of this research was to develop a method to automatically identify geometric class characteristics of shoe outsoles. Such a method takes the difficult problem of quantifying outsole features in a given population and lays the groundwork for a tractable solution. \mt{Are we alluding to the scanner in this paper, or should I change that to focus on automating what examiners are doing now?} A set of geometric class characteristics was defined, based on feature sets currently used by examiners, to both broadly classify a large variety of shoes and to narrow down similarity into a manageable number and type of features for further use in a shoeprint analysis. \fix{I think our conclusions of overlap make this statement too rosy: The final set of features used in this research achieves both goals quite well, and has potential for much finer-grained analysis when recognized features are combined with spatial information and relationships.}

% Briefly summarize methods
% Overview of results: model generally performs well
%The model was developed using the \mt{pre-trained} convolutional base of VGG16 and a newly trained classifier/head to output predicted probabilities within the defined set of geometric features. After training, the model performs well on the data provided. In general, the model is able to identify many well-defined geometric shapes in the images.

% Discussion of weaknesses
\fix{Although model predictions are overall quite good, there are a number of \fix{issues}. Firstly, the model has substantial difficulty predicting images with low contrast because image features are obscured; exploring methods of color correction, such as histogram normalization, may prove useful for eliminating the effect of contrast on predictions.

A larger difficulty is the inherent overlap between features of common geometric shapes, and high variability of those shapes on outsoles. Overlapping features between shapes makes it more difficult for the model to distinguish between geometrically similar shapes, such as chevrons and triangles. When the physical shapes present within a category are highly variable, this further widens the sets of features associated with individual categories, leading to even more model confusion. These issues of overlap and variabailty are at least in part due to the features used by practitioners, and partially due to our generalization/operational definition.}

\mt{What is the balance between comprehensive and redundant? I don't think I've found it... I'll try again when not exhausted}

\mt{Final paragraph will suggest redefining shapes (yuck) and cutting off the final conv block to see whether those features are easier for the model to sort between categories.}
