...
\svp{You need to talk about the model structure of vgg16 before you start talking about layers. Talk of convolutional blocks doesn't really make sense until you show the layer structure.}
\mt{I agree, but I thought that this was where we were arguing that the ``eyes" are transferrable, so I wanted to talk about the filters to back that up.}
\autoref{fig:vgg16layers} shows a selection of filters from VGG16, where each filter detects the presence of specific shapes and colors. Each row of filters in \autoref{fig:vgg16layers} corresponds to one of the five convolutional blocks of VGG16. Filters in earlier convolutional blocks detect simple features, such as colors and blobs, while later filters detect more complex combinations of features. Each of the five convolutional blocks has between 128 and 1,536 filters, and a single image is made up of some combination of the thousands of features that are detected by those filters.
...
f_l <- image_append(c(b, f)) %>%
  image_annotate("Block 1:", location = "+60+159",
                 color = "black", size = 60) %>%
  image_annotate("Block 2:", location = "+60+477",
                 color = "black", size = 60) %>%
  image_annotate("Block 3:", location = "+60+795",
                 color = "black", size = 60) %>%
  image_annotate("Block 4:", location = "+60+1113",
                 color = "black", size = 60) %>%
  image_annotate("Block 5:", location = "+60+1431",
                 color = "black", size = 60) %>%
...
\item [Other] Other features which were marked include logos, various textures (including crepe, stippling, etc.), and smooth regions with no discernible features. These regions are grouped and provide additional information - that none of the previous nine categories are present. \mt{Why no EM dash? ``---"}
\end{description}
...
Although transfer learning substantially reduces the amount of data required to train a CNN by several orders of magnitude, \fix{labeled images are still difficult or time-consuming to generate relative to the amount of data necessary to train even a small neural network adequately. Another common solution to data scarcity is to artificially enlarge the data set using a process called image augmentation \citep{krizhevskyImageNetClassificationDeep2012}.}
\svp{I would lose the last clause of the first sentence and combine it with the next, e.g. to make most efficient use of the labeled data, we enlarge the training data using a process called image augmentation.}
\mt{I don't think the first half of the first sentence is relevant here without the second. Should I use a different transition? I was trying to put augmentation in the context of transfer learning; i.e., transfer learning helps, but getting data is hard, so we use augmentation to get more, with the added benefit of improving generalizability.}
...
\fix{How much should be said about loss and loss functions?}\svp{}
A loss function is a function that measures how far model predictions are from the given labels for each image in the validation and test sets; the model ``learns" by updating its predictions to minimize the loss.

\autoref{fig:training-accuracy} shows the training and validation accuracy and loss at each epoch of the fitting process. Overfitting occurs when a model learns the training data so well that its understanding of the categories becomes specific to the training cases, which, in turn, leads to poor prediction of new images that were not in the training set.
%Overfitting, or fitting a model which performs too well on the training data relative to the validation data,
Overfitting occurs when the validation loss begins to increase after reaching a global minimum. Alternately, underfitting occurs if the validation accuracy is still increasing when model optimization is terminated (for instance, between epochs 1 and 10 in \autoref{fig:training-accuracy}), because model performance is still improving with continued training. In \autoref{fig:training-accuracy}, validation accuracy levels off after epoch 20, and validation loss has not yet begun to increase at epoch 30, indicating that the model optimization process was halted at an appropriate epoch.

\subsection{Model Accuracy}

\mt{\textbf{OUTLINE OF RESULTS}
\begin{itemize}
\item ROC shows that, in general, model performance is better than chance and broadly compares class performance.
\item Show model predictions primarily to provide a few examples to back up the claim that predictions generally look good, but also include one or two case studies that motivate class-by-class diagnostics
\item Confusion matrix shows which classes are most accurate or most problematic. However, when we see that one shape is commonly misclasified as another, the conf mat doesn't let us do anything more than speculate the cause.
\item Heatmaps help us look at specific instances of classification to 1) verify that the model is using appropriate features when it classifies correctly and 2) diagnose the cause of confusion when the model classifies incorrectly
\item (Potentially switch the order of prediction examples and the confusion matrix, so the conf mat indicates systemic class confusion and then back that up with the examples?)
\end{itemize}
}
...
For each image in the test set, the ``true labels" are nine human-assigned labels of 0 or 1 corresponding to \svp{the presence or absence of} each of the nine shape categories \svp{in the image}. When the CNN predicts which shapes are in the image, however, it assigns a probability between 0 and 1 for each shape category, with a total of nine probabilities per test image. Thus, to determine the accuracy of the model predictions, we must select a threshold for each label to discretize the model predictions. %such that a predicted probability above the threshold indicates a ``yes" prediction from the model and a probability below the threshold indicates a ``no".}
...
%\mt{I don't immediately see how to separate the below sentence into two without making it really clunky... I figured that in the context of thresholds (previous paragraph), it would be easy to just stick in that those thresholds are used to compute the curve.}

%Better?: Receiver Operating Characteristic (ROC) curves are a type diagnostic plot which compare the false positive rate to the true positive rate for a classification method at a number of thresholds, and the Area Under the Curve (AUC) quantifies the shape of the ROC curve.

Receiver Operating Characteristic (ROC) curves are a type diagnostic plot which compare the false positive rate to the true positive rate for a classification method.\svp{Make this two sentences so that it's less convoluted}\fix{, computed by comparing the two rates at a number of thresholds, and the Area Under the Curve (AUC) quantifies the shape of the ROC curve.} Perfect prediction would be indicated by a right angle along the upper left corner of the plot, with a corresponding AUC of 1, and a diagonal line with an AUC of 0.5 would indicate that the classification method performs no better than random chance. \autoref{fig:overall-roc} shows the ROC curve for \fix{the model} across all classes, and \autoref{fig:class-roc} shows the curve for each class. The full model has an AUC of 0.88, and the AUC for individual classes ranges from 0.81 (for lines) to 0.91 (for bowtie and text). While the class performances do vary slightly, each ROC curve is the same general shape and performs significantly better than random chance. The points in \autoref{fig:class-roc} represent the equal error rate (EER), which is the threshold where there is equal probability of a false positive or false negative error. We will use the class-wise EER to set a threshold corresponding to each class, so that any model prediction with probability above the EER for that shape is considered a positive prediction.
...
\svp{I'd focus more on the fact that ROC curves are a general summary of the model's performance, but we must also examine the specific cases where the model misclassifies objects to determine if there is a pattern.}
\mt{ROC curves provide a general summary of model accuracy, but they do not indicate which classes are being assigned the false positive or false negative labels when accuracy is not perfect. A confusion matrix is a helpful diagnostic tool to understand which classes are commonly confused for other classes.} A confusion matrix is a \mt{summary of the relationships between} the labels assigned to an image and the labels predicted by the model.

In the confusion matrix presented in \autoref{fig:confusion_matrix}, the values along the diagonal represent the proportion of true positives captured within each category. The off-diagonal values represent the proportion of false positives for each shape pairing; however, since a single image may truly contain multiple shapes, the values have been adjusted to remove the effect of any true positives from the calculation of false positive proportions. \svp{As a result of this adjustment, the probabilities off-diagonal are the proportion ... need to explain the conditional probability we're showing here} %\svp{I can't remember exactly how we're doing that, but we may want to look more closely at that - if we set everything to NA instead of 0, the rate calculations would be higher than expected} \mt{They're being set to NA, and then we're calculating proportions with na.rm = T. I don't understand what you mean about higher rates than expected...}.
...
\svp{I've moved this down here because I think it fits better in the model exploration section - using this to explain why performance differs - intraclass variability.}
