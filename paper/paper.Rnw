...
The ROC curves provide only a general summary of model accuracy; to learn more about how specific images are classified, we can examine images from the test set along with their respective nine model predictions. \autoref{fig:single-exemplars} shows model predictions for images that contain only one type of shape, and \autoref{fig:multi-exemplars} shows similar predictions for images containing multiple shapes. Most predictions for the single-label images are straightforward; all of the archetypical shapes are corectly identified by the model, with only small confusions like those between quad/polygon and circle/text. \fix{A later section will discuss possible causes for this confusion.}
...
For example, to calculate the proportion of images that contain triangles but are being falsely labeled as containing quadrilaterals, any image that truly contains both triangles and quadrilaterals is removed before calculating the proportion of false quadrilateral labels.
...
%\svp{I would move heatmaps up and out of the diagnostic section and transition into that, possibly}

%\svp{Three main model exploration chunks: consistency, heatmaps, brand (motivated by the fact that some classes perform so badly because the exemplars are so variable) - need to figure out the best sequence to transition}
...
\fix{Susan, please generate these heatmaps and move them to the "static" folder. My computer still has issues with the transparent layer of the heatmap, so it's probably easiest to do what we did for my CC and have you generate the images so they turn out correctly. Once I can see them all, I'll decide which three heatmaps best illustrate the point that shape categories have similar features that (understandably) confuse the model and I'll write about those specific heatmaps.}

<<setup-heatmap>>=
imgs <- list.files(file.path(wd, "images", "heatmaps", "raw"), full.names = T)
@

<<generate-heatmaps, echo = F>>=
