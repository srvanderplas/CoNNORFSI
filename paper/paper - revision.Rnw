...
In the US, real forensic analysis of shoe print evidence is varied and difficult. It is often impossible to conduct an individualizing analysis when a print is partial or low quality. Beyond the question of identification, it is more common (and arguably more tractable) to consider class characteristics. At the class level,  one question of interest \mt{motivate this more?} is calculating random match probability: Given a questioned shoe print, how many shoes in the relevant population have a similar tread pattern? We consider this question as distinct from make and model determination, since many shoes of different makes and models can still have very similar tread patterns (e.g., knockoffs, same models over different years, etc)

From a statistical perspective, random match probability could be calculated by sampling many shoes from a given population and counting the number of shoes in the sample that are "similar" to the given print. In practice, one potential method of sampling would be a low-profile scanner placed in a high traffic area. Such a scanner could take pictures of shoes walking past, and those images could then be used to create a database of shoes relevant to the local area. \mt{Are cameras more robust/weatherproof than pressure scanners? Why images instead of EverOS impressions?} \mt{What is the best way to say that such a scanner already exists, and/or that the grant is approved?} Assuming that such a database of images is available, the task becomes assessing the similarity of prints. This is done by identifying relevant features in the scans and using image analysis methods to detect the presence or absence of the features in each image, and then defining similarity based on those features to determine how many shoes have a tread pattern similar to the pattern of interest.
