...
The \Sexpr{nrow(dfunion)} images were split such that 60\% were used for training. Since \mt{some categories are much more common than others,}\svp{I like the equal proportion phrasing better myself}
%the categories do not exist in equal proportion in the labeled data,
the training data were weighted by proportion during the training process to \svp{ensure that the model can identify both rare and common geometric shapes}.%from being overwhelmed by more frequent categories.
Of the remaining 40\% of data, half were used for validation, to monitor the training process, and the remaining data were for testing the performance of the fitted model.
...
\svp{get rid of passive voice here. Overfitting occurs when the validation loss begins to increase after reaching a global minimum} Overfitting is suspected if the validation loss starts to increase after reaching a global minimum. Alternately, underfitting occurs if the validation accuracy is still increasing when model optimization is terminated\svp{(for instance, between epochs 1 and 10 in \autoref{fig:training-accuracy})}, because model performance is still improving with continued training. \svp{In \autoref{fig:training-accuracy}, validation accuracy levels off after epoch 20, and validation loss has not yet begun to increase at epoch 30,} indicating that the model optimization process was halted at an appropriate epoch.
...
\autoref{fig:confusion_matrix} shows \svp{systematic errors in class predictions}, but does not necessarily suggest \emph{why} the confusion is taking place. To assess the reasons for model failure, \svp{we look at the images to} \fix{see what the model is seeing.}
