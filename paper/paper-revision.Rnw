...
Although footwear impressions are more commonly used in practice for footwear analysis than outsole images, early prototypes of a footwear scanner have suggested that cameras are more robust than pressure scanners for placement in public locations that may experience inclement weather or other non-controlled conditions. Thus, \fix{we move forward by considering how to assess similarity of tread patterns in images, in particular.} More specifically, this assessment is done by identifying a set of relevant features and using image analysis methods to detect the presence or absence of the features in each image, and then defining similarity measures based on those features to determine how many shoes have a tread pattern similar to the pattern of interest.

\fix{We are working to dev a scanner using photo technology. It takes pics like Zappos. While images are degraded in some ways, the fundamental reqs are sim. We want to dev the methodology for this before we get actual data.}

\fix{manual collection citations: presentation by Sandy Parent, benedict et al, benedict cites another, the one that says college students and criminals are basically the same}
