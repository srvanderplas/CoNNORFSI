...
%\autoref{fig:confusion_matrix} shows systematic errors in class predictions, but does not necessarily suggest \emph{why} the confusion is taking place. To assess the reasons for model failure, we look at the images to \fix{see what the model is seeing.}
