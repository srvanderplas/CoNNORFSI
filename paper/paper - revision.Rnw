...
\mt{General paper to-do:}
\mt{1. Find a paper to cite the 95\% statistic}
\mt{Option: Shoe marks are most common at a crime scene (Rida et al., 2019)}
\mt{2. A zotero reference is missing details, remember to go find/fix it}
\mt{3. Cite papers with manual data collection}
\mt{3.5. Incorporate recommended citations from reviewer 3}
\mt{4. Susan will send pictures of the prototype scanner from the grant proposal}
\mt{5. Argue in methods that we know real image quality will be lower but we needed to start somewhere}
\mt{6. Our goal is to narrow down what type of model works and assess the model(s) limitations}
\mt{7. Add to conclusion: Overall, this concept seems to work, but we need to modify the classificiation scheme. Thus, this work is publishable, because it needs work and community input. Need more research into human comphrehensible and yet useful classification schemes for this purpose. We need better class scheme, important and promising, getting this right matters}
\mt{8. Update abstract to reflect new framing of paper}
\mt{}
