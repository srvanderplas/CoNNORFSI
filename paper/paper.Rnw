...
<<low-contrast-consistency, fig.cap = "Model predictions for images with low contrast. While these images contain a variety of shapes \\svp{(as shown in the color-corrected column on the right)}, the diffuse probabilities assigned by the model across all categories indicate that features are obscured by low contrast, which propogates through model training and prediction.", fig.pos = "p!", out.width = "100%", fig.width = 10/1.2, fig.height = 9/1.2, cache = T>>=
