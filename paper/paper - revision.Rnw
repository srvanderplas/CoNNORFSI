...
From a statistical perspective, random match probability could theoretically be calculated by sampling a large number of shoes from a given population and counting the number of shoes in the sample whose tread patterns are similar to the given print. In practice, one potential method of sampling could be a low-profile scanner placed in a high traffic area. Such a scanner could take \mt{impressions or} pictures of shoes walking past, and those scans could then be used to create a database of shoes relevant to the local area.

However, studies by \fix{authors (yr)} and \fix{authors (yr)} demonstrate that even \fix{[a couple hundred]} shoe prints constitute an unweildy amount of data requiring \fix{[a large amount of data processing]}. Thus, in parallel with the development of such a scanner, it is necessary to develop a way to process large amounts of data automatically. \mt{The primary focus of this paper is to develop one such method for large-scale automatic data processing; in this case, "processing" refers to assessing} the similarity of the shoe tread patterns in the collected data. %\fix{Cut this?: This is done by identifying a set of relevant features and using image analysis methods to detect the presence or absence of the features in each image, and then defining similarity measures based on those features to determine how many shoes have a tread pattern similar to the pattern of interest.}

Although footwear impressions are more commonly used in practice for footwear analysis than outsole images, early prototypes of a footwear scanner have suggested that cameras are more robust than pressure scanners for placement in public locations that may experience inclement weather or other non-controlled conditions. Thus, \fix{this is an image recognition problem}.
...
There is precedent for using machine learning to identify tread patterns \mt{hunt through papers to find relevant references}
