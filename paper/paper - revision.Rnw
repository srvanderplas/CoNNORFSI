...
In the United States, forensic analysis of shoe print evidence in criminal cases is varied and difficult. The process of collecting print evidence from a crime scene is not standardized, and it is often impossible to conduct an individualizing analysis when a print is partial or of low quality. In fact, some experts estimate that 95\% of cases in the United States do not consider individualizing characteristics of shoeprint evidence \footnote{Leslie Hammer, presentation to CSAFE on March 5, 2018}). Even so, in these cases it still may be useful, and much more tractable, to assess the random match probability of the footwear evidence. In other words, given a questioned shoe print, what is the probability that a randomly selected shoe in the relevant population has a similar tread pattern? Note that this question is distinct from make and model determination since many shoes of different makes and models can still have similar tread patterns (e.g., knockoffs, same models over different years, etc).


From a statistical perspective, random match probability could be calculated by sampling a large number of shoes from a given population and counting the number of shoes in the sample whose tread patterns are similar to the given print. In practice, one potential method of sampling could be a low-profile scanner placed in a high traffic area. Such a scanner could take pictures of shoes walking past, and those images could then be used to create a database of shoes relevant to the local area. \mt{Are cameras more robust/weatherproof than pressure scanners? Why images instead of EverOS impressions?} \mt{(Note: a grant proposal for such a scanner has been approved by NIJ, and there currently exists a working prototype.)} Assuming that such a database of images is available, the task becomes assessing the similarity of the shoe tread patterns in the images. This is done by identifying a set of relevant features and using image analysis methods to detect the presence or absence of the features in each image, and then defining similarity measures based on those features to determine how many shoes have a tread pattern similar to the pattern of interest.

CNNS have become the gold standard of image recognition. \mt{salvage the other intro to explain why simpler methods aren't sufficient.} There is precedent for using machine learning to identify tread patterns \mt{hunt through papers to find relevant references}
