...
<<overall-roc, fig.width = 5, fig.height = 5, out.width = ".5\\textwidth", fig.cap = "ROC curve showing overall model performance. \\mt{The curve indicates that model accuracy is generally high.}", fig.scap = "Overall model performance ROC curve.">>=
library(pROC)
...
<<exemplars, fig.cap = "\\mt{Model predictions of images containing a single shape, no shape, or multiple shapes across all categories. Although there are some confusions (such as between circle and text in the single class images, and the false positive prediction of triangle in the image with green text), predictions are generally reasonable and consistent with the human-assigned labels of these images.}", fig.pos = "p!", out.width = "100%", cache = T, fig.width = 8, fig.height = 12>>=
...
<<circle-text-consistency, fig.cap = "\\mt{Model predictions of images containing circles and/or text. The model correctly identifies circles in all images where circles are present, but also predicts moderate probabilities of circles in many images where text is present with curved letters, such as ``G\". Text is correctly identified in most images, with the exception of the false negative prediction of the low-contrast Teva image and the false positive prediction of concentric circles. In general, model predictions are reasonable but suggest a few sources of systematic misclassification, which are discussed in a later section.}", fig.pos = "p!", out.width = "100%", cache = T, fig.width = 8, fig.height = 12>>=
