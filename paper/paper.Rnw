...
<<exemplars, fig.cap = "Examples containing a single shape", fig.pos = "p!", out.width = "100%", cache = F>>=
...
<<circle-text-consistency, fig.cap = "Text with circles or circle-like shapes", fig.pos = "p!", out.width = "100%", cache = F>>=
...
The horizontal band in \autoref{fig:confusion_matrix} indicates that quadrilaterals are predicted more often than they should be for every true label. Similarly, polygons, stars, and triangles produce a large number of false positive predictions into other categories, as evidenced by the\mt{ir} vertical bands. It is also apparent that circles and text are commonly confused, which supports Another interesting artifact in \autoref{fig:confusion_matrix} is that circles are often misclassified as text, and text is often mislabeled as containing circles.
