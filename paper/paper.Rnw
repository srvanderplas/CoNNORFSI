...
<<exemplars, fig.cap = "Model predictions of images containing \\svp{(from the top)} a single shape, no shape, or multiple shapes across all categories. \\svp{Predicted probabilities greater than the equal error rate threshold are outlined in black.} Although there are some \\svp{misclassifications}, predictions are generally consistent with the human-assigned labels of these images.", fig.pos = "p!", out.width = "100%", cache = T, fig.width = 11.5/1.25, fig.height = 16/1.25>>=
